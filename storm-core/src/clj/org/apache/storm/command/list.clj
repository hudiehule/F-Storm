      ...
      (if (or (nil? topologies) (empty? topologies))
        ...
        (do
          ...
          (doseq [^TopologySummary topology topologies]
            (let [topology-name (.getName topology)
                  topology-status (.getStatus topology)
                  topology-num-tasks (.getNum_tasks topology)
                  topology-num-workers (.getNum_workers topology)
                  topology-uptime-secs (.getUptime_secs topology)]
